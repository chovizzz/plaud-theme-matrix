...
| 项 | 值 |
|---|---|
| 矩阵包版本 | **v0.3.5**（已发布，git tag `v0.3.5`） |
| 包名 | `plaud-theme-matrix`（git 仓库 <https://github.com/chovizzz/plaud-theme-matrix>；版本靠 tag 钉，不再有 `-vX.Y.Z` 目录与 zip） |
| 契约版本（`ContractVersion`） | **v0.3.5**（与包版本同步递增；v0.3.2–v0.3.5 都只动包内附带工具 skill 与安装/更新机制（v0.3.5 另在本 skill 加了一节非契约的更新提示），矩阵字段、枚举与路由一字未改——但契约版本仍按约定同步递增，否则四端会判版本漂移） |
