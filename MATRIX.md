...
`plaud-shopify-theme-matrix-v0.1.0` directory is a distribution package, **not** an
...
| 6 | `plaud-theme-qa` | Verify 阶段：Theme Check baseline 增量、5 断点回归、多语言、A11y、红线核查 —— **唯一交付权** | 用户 |
...
| Implement | dev / section-build / ux-migration | `ChangeSetId`（`CS-<YYYYMMDD>-<path><NN>`）、`ModifiedFiles`、`BaseHeadSha`、`ChangeSetFingerprint`、`RequiredQAProfile`（共 18 字段） | `plaud-theme-qa` |
| Verify | `plaud-theme-qa` | `ChangeSetIdMatched`、逐项 Passed/Failed/Blocked/NotApplicable、`ReadyForDelivery`（共 19 字段） | 用户 / 回退到实现 skill |
...
flowchart TD
  ...
  build --> cs
  mig --> cs
  cs --> qa[plaud-theme-qa]

  qa -->|ChangeSetIdMatched: No| stop
  qa -->|任一项 Failed| back[退回实现 skill 修复 · 重新生成 ChangeSetId]
  back --> cs
...
单个 bug → `plaud-theme-dev`。但任何全流程/跨块工作都必须过 `plaud-theme-shared` 契约，
...
改动前后各跑一次 `shopify theme check`，只比对 `ModifiedFiles` 范围内的新增 offense，
新增为 0 才 `Passed`。判定细节与 Blocked 的合法情形见 handoff-schema §6。
